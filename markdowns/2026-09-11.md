# 🚩 (2026-09-11) Scholar Inbox 추천 논문 
...
## 🌏 Abstract (번역)
오디오북 내레이션, 대화형 에이전트, 시청각 더빙은 단일 발화 내에서 변화하는 감정을 전달하고 속도를 조절하는 음성을 필요로 합니다. 그러나 대부분의 기존 TTS 시스템은 발화 단위의 스타일 조건화에 의존하여 이러한 미세한 제어를 달성하기 어렵습니다. 이에 따라, 대규모 언어 모델의 후속 학습(post-training) 성공에 영감을 받아, 사전 학습된 텍스트-음성 변환 모델에 세그먼트 수준의 감정과 길이에 대한 자연어 제어 기능을 부여하는 통합 후속 학습 프레임워크를 제안합니다. 지도 미세 조정(supervised fine-tuning)은 지시 조건부 음성 생성을 확립하고, 그룹 상대 정책 최적화(group relative policy optimization)를 사용한 강화 학습은 내용 및 화자 보존 목표와 함께 감정 및 길이 보상을 사용하여 제어 정확도를 개선합니다. 사전 학습된 아키텍처를 재사용함으로써, 우리의 접근 방식은 추가적인 추론 시간 제어 모듈을 피합니다. 실험은 음성 명료도와 화자 정체성을 유지하면서 미세한 제어 가능성이 크게 향상되었음을 보여주며, 후속 학습이 기존 음성 합성 모델을 확장하는 실용적인 접근 방식임을 강조합니다.

## 🔍 Methods & Results
- 사전 학습된 Cosyvoice2 음성 언어 모델을 기반으로, 텍스트를 스팬(span)으로 나누고 자연어 지시(c)를 통해 각 스팬의 감정, 길이 또는 상대적 발화 속도를 제어하여 단일 발화를 생성하는 것을 목표로 합니다.
- 감정 데이터는 MED-TTS의 5가지 감정 범주를 IndexTTS2로 합성하고, Seed-TTS의 화자 참조 및 ESD의 감정 참조(수동 선별 및 MERaLiON-SER-v1 분류 신뢰도 고려)를 활용하여 구성했습니다.
- 길이 및 공동 제어 데이터는 LibriTTS-R 녹음에서 음성 세그먼트를 PSOLA를 사용하여 독립적으로 늘리거나 압축하여 구성했으며, 측정된 세그먼트 길이와 적용된 길이 비율로부터 정확한 길이 및 상대적 속도 지시를 생성했습니다. 이 방법은 중국어 평가 세트에도 일반화되었습니다.
- 지도 미세 조정(SFT)을 통해 모델이 로컬 감정 지시를 음성으로 매핑하는 방법을 학습시키며, 생성된 훈련 오디오에서 추출된 목표 음성 토큰(z*)에 대해 음성 토큰 정책(πθ)을 최적화합니다.
- 감정 중심 GRPO(Group Relative Policy Optimization)는 SFT 모델에서 시작하여 감정 제어를 더욱 최적화합니다. Qwen3-ForcedAligner로 생성된 음성을 정렬하고 MERaLiON-SER-v1로 감정을 예측하며, 감정 신뢰도, 세그먼트 수준 정확도, 발화 수준 정확도, ASR 기반 내용 일관성 및 화자 유사성을 결합한 보상을 사용합니다.
- 길이 SFT는 감정 정제 모델을 길이 제어 데이터에 대해 미세 조정하며, 음성 토큰 예측과 세그먼트 토큰 길이 및 상대적 경계에 대한 보조 감독(smooth-L1 손실)을 결합합니다.
- 길이 중심 GRPO는 Qwen3-ForcedAligner로 세그먼트 길이를 측정하고, 정확한 길이 요청(목표 및 생성된 길이 기반의 부드러운 점수) 및 상대적 속도 요청(간격 준수 및 목표 비율 근접성)에 대한 보상을 사용합니다. 이 보상은 세그먼트/발화 수준 정확도 및 누적 경계 일치와 결합됩니다.
- 공동 정제 단계에서는 감정 전용, 길이 전용 및 공동 제어 예제를 혼합하여 감정 제어를 회복하고 타이밍 준수를 통합하며, 내용 및 화자 보상은 모든 작업에 적용되고, 감정 및 길이 보상은 해당 작업에만 적용됩니다.
- 지시 동등성 증류(Instruction-equivalence distillation) 단계는 지시어 표현에 대한 견고성을 향상시키기 위해, 정규 지시(c)와 동일한 제어 의도를 가진 의역(c~)을 사용하여 학생 모델을 최적화합니다.
- 마지막으로, 증류된 모델에서 시작하여 혼합된 감정, 길이 및 공동 제어 예제에 대해 짧은 GRPO 단계를 적용하여 제어 정확도를 회복하고 추가로 개선합니다.

---
**Usage Info**: 3513 tokens used.
**Generated at**: 2026-09-11 17:29:54
...
## 🌏 Abstract (번역)
텍스트 음성 변환(TTS) 모델은 일반적으로 예측된 길이를 사용하여 음소 수준 인코더 상태를 프레임 수준 디코더 입력으로 확장함으로써 텍스트-음성 정렬을 처리합니다. 이러한 길이 조절 단계는 구조적으로 정렬을 해결하지만, 이러한 길이 사용은 잠재 상태가 나타나는 위치와 빈도만 변경할 뿐, 상태 자체의 값은 변경하지 않습니다. 본 논문은 신경 제어 미분 방정식(CDE)을 사용하여 TTS에서 길이 인지 음향 모델링을 위한 연속 시간 메커니즘을 제안합니다. 우리는 음소 표현을 시간적으로 매개변수화된 제어 경로로 공식화하고, 신경 음향 벡터 필드를 사용하여 음성 내용과 길이에서 파생된 타이밍에 따라 값이 진화하는 연속 시간 은닉 상태를 생성합니다. 결과 궤적은 이산 지점에서 샘플링되어 표준 음향 디코더 파이프라인에 통합될 수 있습니다. 객관적인 결과는 CDE와 일반적인 순환 모델을 대조합니다. 주관적인 결과는 단계당 하나의 음소를 평가하는 CDE 기반 모델이 강력한 기준선과 유사한 감정 표현 품질을 유지하면서 합성된 감정 강도와 참조 감정 강도 간의 순위 일치도를 향상시킬 수 있음을 시사합니다. 절반 음소 단계 크기를 사용한 추가 실험은 시간 해상도가 스타일 추적과 절대 보정 간의 균형을 변화시킨다는 것을 보여줍니다. 이러한 결과는 CDE를 연속 시간 및 길이 인지 스타일 민감 TTS를 위한 유망한 설계 공간으로 자리매김합니다.

## 🔍 Methods & Results
- 기존 TTS 모델은 예측된 길이를 사용하여 음소 수준 인코더 상태를 프레임 수준 디코더 입력으로 확장하지만, 이는 상태의 값 자체를 변경하지 않고 나타나는 위치와 빈도만 조절합니다.
- 길이 조절은 반복을 통해 지속 시간을 표현하므로, 디코더가 지역 발화 속도, 음소 경계 근접성, 발화 내 시간 진행과 같은 지속 시간 의존적 정보에 접근하기 어렵게 만듭니다.
- 본 논문은 신경 제어 미분 방정식(CDE)을 사용하여 TTS에서 지속 시간 인지 음향 모델링을 위한 연속 시간 메커니즘을 제안합니다.
- 음소 표현은 시간적으로 매개변수화된 제어 경로(X)로 공식화되며, 신경 음향 벡터 필드(F_theta)는 음성 내용과 지속 시간 파생 타이밍에 따라 값이 진화하는 연속 시간 은닉 상태(Z)를 생성합니다.
- 제어 경로(X)는 정렬(A)에서 파생된 시간 채널로 보강되어, CDE가 지속 시간 정보를 디코더에 연속적으로 통합하도록 합니다.
- CDE는 시간 변화에 민감하며, 아조인트 상태 방법을 사용하여 역전파를 수행하여 메모리 요구 사항을 줄일 수 있습니다.
- CDE 레이어는 체인 규칙을 통해 효율적으로 쌓을 수 있어, 중간 궤적을 명시적으로 구체화하거나 재보간할 필요 없이 여러 연속 시간 조건화 레이어가 동일한 지속 시간 증강 제어에 대해 작동할 수 있습니다.
- 객관적인 결과는 CDE가 일반적인 순환 모델과 대조됨을 보여줍니다.
- 주관적인 결과에 따르면, 단계당 하나의 음소를 평가하는 CDE 기반 모델은 합성된 감정 강도와 참조 감정 강도 간의 순위 일치도를 향상시키면서 강력한 기준선과 유사한 감정 표현 품질을 유지합니다.
- 절반 음소 단계 크기를 사용한 추가 실험은 시간 해상도가 스타일 추적과 절대 보정 간의 균형을 변화시킨다는 것을 시사합니다.
- 이러한 결과는 CDE가 연속 시간 및 지속 시간 인지 스타일 민감 TTS를 위한 유망한 설계 공간임을 나타냅니다.

---
**Usage Info**: 4149 tokens used.
**Generated at**: 2026-09-11 17:30:04
...
## 🌏 Abstract (번역)
이차 차이음(QDT)은 음향 신호에는 존재하지 않지만 청취자에게는 명확하게 들리는 '환영' 순음의 일종인 청각 왜곡 산물입니다. 이 현상을 활용하여 음악적 목적을 위한 배음이 풍부한 음색을 합성할 수 있는데, 이를 이차 차이음 스펙트럼(QDTS) 합성 기술이라고 합니다. 이전 연구들은 목표 QDTS와 배음 구조의 캐리어 신호를 연결하는 왜곡 함수를 기반으로 QDTS를 합성하는 수치적 방법을 제시했습니다. 이러한 방법들은 정확했지만, 확률적이고 불연속적이어서 음악적 목적으로 제어하기 어렵고 사실상 정지 신호에만 국한되었습니다. 본 논문은 오토인코더와 유사한 구성으로 왜곡 매핑의 근사 역함수를 학습하는 신경망 기반 접근 방식을 제안하여, 이전의 한계점을 해결하는 연속적인 근사를 생성합니다. 실험 결과에 따르면, 이 방법은 수치적으로는 약간 덜 정밀하지만 지각적 및 음악적 응용에는 충분합니다. 또한 Max에서 실시간 버전을 구현하고 성능을 평가했습니다. 다양한 사운드 예시는 이 방법의 표현력과 음악적 잠재력을 보여줍니다. 본 연구에 수반되는 소스 코드, 오디오 예시, 튜토리얼 및 소프트웨어는 https://cordutie.github.io/projects/qdts.html에서 확인할 수 있습니다.

## 🔍 Methods & Results
- 기존 QDTS 합성 수치적 방법은 정확했지만 확률적이고 불연속적이며, 음악적 제어가 어렵고 정지 신호에만 국한되는 한계가 있었다.
- 본 논문은 오토인코더와 유사한 구성으로 왜곡 매핑의 근사 역함수를 학습하는 신경망 기반 접근 방식을 제안하여, 이전의 한계점을 해결하는 연속적인 근사를 생성한다.
- 실험 결과, 제안된 방법은 수치적으로는 약간 덜 정밀하지만, 지각적 및 음악적 응용에는 충분함을 입증했다.
- Max에서 실시간 버전을 구현하고 성능을 평가했으며, 다양한 사운드 예시를 통해 이 방법의 표현력과 음악적 잠재력을 시연했다.

## 🖼 Figures
![Figure 3:A snapshot of the developed Max patch to allow for easy interaction with the qdts.solver_nn external employing the trained models.](../images/2026-09-11/2609.10913/2609.10913_fig2.png)
...
---
**Usage Info**: 2286 tokens used.
**Generated at**: 2026-09-11 17:30:14
...
## 🌏 Abstract (번역)
신경망 오디오 코덱은 현대 음성 생성 시스템의 근본적인 구성 요소입니다. 최근 코덱들이 점점 더 낮은 비트레이트를 달성하고 있지만, 프레임 레이트를 줄이는 것은 각 토큰이 재구성 품질을 유지하면서 더 많은 정보를 보존해야 하므로 여전히 어렵습니다. 본 논문에서는 6.25Hz 및 0.80kbps로 작동하며 이론적 지연 시간이 160ms인 스트리밍 신경망 음성 코덱인 ZipCodec을 소개합니다. 우리의 접근 방식은 대규모 WavLM 증류(distillation)와 재설계된 트랜스포머 기반 아키텍처, 스칼라 구형 양자화기(scalar spherical quantizer), 그리고 지연 시간 인식 스트리밍 디코더를 결합합니다. 실험 결과 ZipCodec은 유사한 비트레이트에서 기존 스트리밍 코덱보다 재구성 및 다운스트림 작업 모두에서 훨씬 뛰어난 성능을 보였으며, 훨씬 낮은 프레임 레이트에서 작동합니다. 8억 4천 2백만 개의 파라미터에도 불구하고, ZipCodec은 소비자용 CPU에서 실시간 단일 스트림 추론을 달성합니다. 데모 샘플, 코드 및 체크포인트는 https://lucadellalib.github.io/zipcodec-web/에서 확인할 수 있습니다.

## 🔍 Methods & Results
- ZipCodec은 6.25Hz 및 0.80kbps로 작동하며 160ms의 이론적 지연 시간을 가진 스트리밍 신경망 음성 코덱으로 제안되었다.
- 방법론은 대규모 WavLM 증류, 재설계된 트랜스포머 기반 아키텍처, 스칼라 구형 양자화기, 그리고 지연 시간 인식 스트리밍 디코더를 결합한다.
- 실험 결과, ZipCodec은 유사한 비트레이트에서 기존 스트리밍 코덱보다 재구성 및 다운스트림 작업 모두에서 훨씬 뛰어난 성능을 보였으며, 훨씬 낮은 프레임 레이트에서 작동한다.
- 8억 4천 2백만 개의 파라미터에도 불구하고, ZipCodec은 소비자용 CPU에서 실시간 단일 스트림 추론을 달성한다.

## 🖼 Figures
![Figure 1:ZipCodec architecture. The encoder extracts features containing both acoustic and semantic information. These features are then mapped to a low-dimensional space by the compressor, quantized, and projected back by the decompressor. The decoder resynthesizes the waveform from these features. All these modules are causal, while a non-causal teacher is used for distillation to align causal features with their non-causal counterparts.](../images/2026-09-11/2609.11642/2609.11642_fig0.png)
...
---
**Usage Info**: 1406 tokens used.
**Generated at**: 2026-09-11 17:30:20
...
## 🌏 Abstract (번역)
두 화자의 악센트 차이를 결정하는 것은 언어학 및 음성 기술 연구의 기본적인 과제입니다. 이러한 차이를 측정하는 데 사용되는 방법론은 특정 연구 분야에 따라 달라집니다. 음성학 연구자는 개별 단어의 짝을 이룬 녹음에서 모음 포먼트를 비교하여 악센트 변화를 보여줄 수 있습니다. 이러한 결과는 해석 가능하지만, 녹음 수집에 시간이 많이 걸리고 연결된 발화를 대표하지 못할 수 있습니다. 악센트가 있는 텍스트 음성 변환(TTS) 연구는 악센트 분류 작업에서 파생된 악센트 임베딩을 사용하는 방향으로 나아가고 있습니다. 이러한 임베딩은 모든 음성 녹음에서 생성될 수 있지만, 쉽게 해석할 수 없습니다. 본 논문에서는 조음 역변환을 통해 생성된 조음 표현이 악센트 비교를 위한 해석 가능한 기반으로 사용될 수 있으며, 최적 수송(optimal transport)이 임의의 녹음 유형에 걸쳐 악센트 비교를 위한 프레임워크를 제공함을 보여줍니다.

## 🔍 Methods & Results

## 🖼 Figures
![Figure 1:Normalised distances between the target speaker (red star) and all other speakers according to alignment and OT with articulatory features (Left, Middle) and GenAID embedding distance.](../images/2026-09-11/2609.11458/2609.11458_fig0.png)
...
![Figure 2:Tongue position for Rhotic and Non-Rhotic speakers at maximum alignment error in the word ‘airport’ (Left) and ‘store’ (Right).](../images/2026-09-11/2609.11458/2609.11458_fig1.png)
...
![Figure 3:Rhotic (star) and non-rhotic (cross) tongue positions for ‘airport’ and ‘store’ compared with articulatory clusters coloured by intensity according to OT distance.](../images/2026-09-11/2609.11458/2609.11458_fig2.png)
...
![Figure 4:Effect of cluster size and number of minutes input on alignment distance SRC and classification accuracy.](../images/2026-09-11/2609.11458/2609.11458_fig3.png)
*Figure 4:Effect of cluster size and number of minutes input on alignment distance SRC and classification accuracy.*


---
**Usage Info**: 1466 tokens used.
**Generated at**: 2026-09-11 17:30:27
...
## 🌏 Abstract (번역)
본 논문은 연속적인 자기회귀 음성 모델에서 제어 가능한 비언어적 발성(NVV) 생성을 위해 대규모 오디오-언어 모델(LALM) 피드백을 활용한 선호도 최적화 프레임워크를 제안합니다. 인간의 선호도 주석 없이 선호도 데이터를 구축하기 위해, NVV가 주입된 실제 전사본과 LLM이 생성한 의미론적으로 정렬된 프롬프트를 결합하여 이중 언어 프롬프트 코퍼스를 구축하고, 확률적 모델 롤아웃을 수행하며, LALM을 사용하여 후보 발화를 순위 매기고 동일 프롬프트 내에서 선택-거부 쌍을 형성합니다. 그런 다음 두 단계 최적화 전략을 채택합니다. 먼저 RSFT(Rejection Sampling Fine-Tuning)를 통해 LALM이 선택한 고득점 샘플에 모델을 적응시킨 후, 발화 수준의 플로우 매칭 손실을 사용하여 쌍별 선호도 최적화를 공식화하고 선택된 샘플의 플로우 매칭 목표를 SFT 앵커로 유지하는 Anchored Flow-DPO를 적용합니다. 이 설계는 명시적인 시퀀스 우도 없이 DPO 스타일의 선호도 학습을 가능하게 하면서 선호되는 실현에 대한 직접적인 감독을 유지합니다. 공식 1,600개 발화 NVVSpeech Challenge Track 2 테스트 세트에서, 우리의 방법은 최종 Track2Score 75.80(중국어 79.39 / 영어 72.21)을 달성하여 VoxCPM2 기준선보다 +1.84 높은 성능을 보였습니다. 개선 사항은 주로 더 높은 NVV 정확도와 NVV 지각 효과에 의해 주도되었으며, 전반적인 품질은 안정적으로 유지되었습니다. 음성 샘플은 데모 페이지에서 확인할 수 있습니다.

## 🔍 Methods & Results
- 제어 가능한 NVV 생성을 위해 LALM 피드백을 활용한 선호도 최적화 프레임워크를 제안하며, 이는 연속적인 자기회귀 음성 모델에 적용됩니다.
- 선호도 데이터는 이중 소스 프롬프트 구성, 확률적 롤아웃, LALM 기반 평가를 통해 구축됩니다.
- 이중 소스 프롬프트 구성은 5,000개의 이중 언어(중국어-영어) 프롬프트로 이루어지며, 텍스트 의미와 NVV 태그가 충돌하는 2,500개의 프롬프트와 의미론적으로 정렬된 2,500개의 프롬프트로 구성됩니다.
- 각 프롬프트에 대해 기준 모델은 10개의 후보 발화를 생성하며, LALM(Qwen3.5-Omni-Plus)은 NVV 정확도, 지각 효과, 자연스러움, 품질, 표현력 등 5가지 차원에서 각 후보를 평가하여 점수를 매깁니다.
- LALM 점수를 기반으로 각 프롬프트에서 가장 높은 점수를 받은 발화(선택됨)와 가장 낮은 점수를 받은 발화(거부됨)를 선택하여 5,000개의 선호도 쌍을 형성합니다.
- 최적화는 두 단계로 진행됩니다: 1단계는 LALM이 선택한 고득점 샘플에 대해 표준 플로우 매칭 목표를 사용하여 RSFT(Rejection Sampling Fine-Tuning)를 수행하여 선호도 인식 초기화를 얻습니다.
- 2단계는 Anchored Flow-DPO를 수행하며, 이는 쌍별 Flow-DPO 항(발화 수준 플로우 매칭 손실을 사용하여 우도 비율을 대체)과 선택된 샘플에 대한 SFT 항(SFT 앵커)을 결합하여 선호되는 실현에 대한 직접적인 감독을 유지합니다.
- 제안된 방법은 NVVSpeech Challenge Track 2 테스트 세트에서 최종 Track2Score 75.80(중국어 79.39 / 영어 72.21)을 달성하여 VoxCPM2 기준선(73.96)보다 +1.84 높은 성능을 보였습니다.
- 성능 개선은 주로 NVV 정확도와 NVV 지각 효과의 향상에 기인하며, 전반적인 품질은 안정적으로 유지되었습니다.

## 🖼 Figures
![Figure 1:Overview of the proposed LALM-based preference data construction and two-stage preference optimization framework with RSFT and Anchored Flow-DPO.](../images/2026-09-11/2609.11260/2609.11260_fig0.png)
...
**Generated at**: 2026-09-11 17:30:38
...
## 🌏 Abstract (번역)
음성 딥페이크 탐지(SDD) 모델은 텍스트 음성 변환(TTS) 및 음성 변환(VC)과 같은 다양한 스푸핑 시스템을 포함하는 다중 공격 데이터셋으로 훈련됩니다. 다중 공격 데이터셋에 대한 표준 분류기 훈련에서는 모든 공격이 하나의 스푸핑된 클래스로 처리되며, 성능은 전체 등오류율(EER)을 사용하여 보고됩니다. 이러한 집계된 관점은 개별 공격이 학습 및 일반화에 어떻게 영향을 미치는지 모호하게 만듭니다. 이러한 공격 수준의 행동을 더 잘 이해하기 위해, 우리는 먼저 샘플 및 공격 생략을 사용하여 TTS 및 VC 노출의 균형을 맞춥니다. 그런 다음 추론 시 공격별 EER을 측정하고, 최적화를 특성화하기 위해 공격별 훈련 손실 및 예측 엔트로피를 분석합니다. 결과는 공격들이 불균등하게 기여한다는 것을 보여줍니다: 일부 공격은 높은 EER 민감도와 낮은 손실로 집중된 엔트로피를 가지며, 이는 결정 경계에 강한 영향을 미친다는 것을 나타냅니다. 우리는 이를 고영향 공격(high-impact attacks)으로 정의합니다. 공격 전반에 걸친 불균등한 일반화를 줄이기 위해, 우리는 측정된 공격 영향에 기반하여 노출을 단계적으로 조절하는 리플레이 정규화된, 공격 인식 커리큘럼을 제안합니다. ASVspoof 2019, 2021, ASVspoof 5, 및 Fake-or-Real 데이터셋에 대한 실험은 표준 다중 공격 훈련과 비교하여 전반적인 견고성 향상 및 공격 수준 불균형 감소를 보여줍니다.

## 🔍 Methods & Results
- TTS 및 VC 노출의 균형을 맞추기 위해 샘플 및 공격 생략 기법을 사용했습니다.
- 추론 시 공격별 EER을 측정하고, 최적화를 특성화하기 위해 공격별 훈련 손실 및 예측 엔트로피를 분석했습니다.
- 분석 결과, 공격들이 불균등하게 기여하며, 일부 공격(고영향 공격)은 높은 EER 민감도와 낮은 손실로 집중된 엔트로피를 보여 결정 경계에 강한 영향을 미친다는 것을 발견했습니다.
- 공격 전반의 불균등한 일반화를 줄이기 위해, 측정된 공격 영향에 기반하여 노출을 단계적으로 조절하는 리플레이 정규화된, 공격 인식 커리큘럼을 제안했습니다.
- ASVspoof 2019, 2021, ASVspoof 5, 및 Fake-or-Real 데이터셋 실험에서 표준 다중 공격 훈련 대비 전반적인 견고성 향상 및 공격 수준 불균형 감소를 입증했습니다.

## 🖼 Figures
![Fig. 1:Overview of the attack-level impact analysis. Omission-induced EER sensitivity and entropy–loss dynamics are used to characterize low-impact and high-impact attacks in SDD benchmark datasets.](../images/2026-09-11/2609.11763/2609.11763_fig0.png)
...
![(a)Attacks Included: A3, A4, A5, A6](../images/2026-09-11/2609.11763/2609.11763_fig1.png)
*(a)Attacks Included: A3, A4, A5, A6*

![(a)Attacks Included: A3, A4, A5, A6](../images/2026-09-11/2609.11763/2609.11763_fig2.png)
*(a)Attacks Included: A3, A4, A5, A6*

![(b)Attacks Included: A2, A3, A5, A6](../images/2026-09-11/2609.11763/2609.11763_fig3.png)
*(b)Attacks Included: A2, A3, A5, A6*

![(c)Attacks Included: A1, A4, A5, A6](../images/2026-09-11/2609.11763/2609.11763_fig4.png)
*(c)Attacks Included: A1, A4, A5, A6*

![(d)Attacks Included: A1, A3, A5, A6](../images/2026-09-11/2609.11763/2609.11763_fig5.png)
*(d)Attacks Included: A1, A3, A5, A6*

![(e)Attacks Included: A1, A2, A5, A6](../images/2026-09-11/2609.11763/2609.11763_fig6.png)
*(e)Attacks Included: A1, A2, A5, A6*

![(f)Balanced Dataset: Unequal Attacks, Equal Number of Samples](../images/2026-09-11/2609.11763/2609.11763_fig7.png)
*(f)Balanced Dataset: Unequal Attacks, Equal Number of Samples*

![Fig. 3:Replay-regularized attack-aware curriculum for reducing high-impact attack dominance. Step 1 trains the model on 60% of the low-impact data. Step 2 initializes from the best Step-1 checkpoint and adapts to all high-impact attacks, while replaying the remaining 40% low-impact subset with teacher–student regularization.](../images/2026-09-11/2609.11763/2609.11763_fig8.png)
*Fig. 3:Replay-regularized attack-aware curriculum for reducing high-impact attack dominance. Step 1 trains the model on 60% of the low-impact data. Step 2 initializes from the best Step-1 checkpoint and adapts to all high-impact attacks, while replaying the remaining 40% low-impact subset with teacher–student regularization.*


---
**Usage Info**: 3234 tokens used.
...
## 🌏 Abstract (원문)
Reducing audio-encoder depth lowers the inference cost of speech large language models, but removing complete blocks perturbs the embeddings consumed by the decoder and can cause deletion and premature end-of-sequence errors. We introduce X-AuT, a progressive framework that selects layer combinations through short behavioral probes and restores the pruned model through representation alignment, cross-scale distillation, scheduled student-policy supervision, and LoRA finetuning. The language-model backbone remains frozen, while attention LoRA adapters and the tied output embedding adapt during distillation. Training uses the highest-agreement tier from a transcript-consistency pipeline, followed by source reweighting during finetuning. On ten public Chinese–English benchmarks, compressing Qwen3-ASR-0.6B from 18 to 16 audio-encoder layers reduces macro-average error from 5.61% to 5.27%. The 14-layer model reaches 5.75% with 20.7% fewer audio-tower parameters. Under the matched recipe, the 1.7B teacher yields 5.55% mean error, compared with 8.45% for self-distillation, and progressive 18→ightarrow14 pruning outperforms direct pruning (5.75% vs. 6.73%). These single-run results establish two practical operating points and show that the accuracy effects vary across benchmarks. Project website:https://xpeng-ai.github.io/x-aut.
## 🌏 Abstract (번역)
오디오 인코더 깊이를 줄이면 음성 대규모 언어 모델의 추론 비용이 낮아지지만, 전체 블록을 제거하면 디코더가 소비하는 임베딩이 교란되어 삭제 및 조기 시퀀스 종료 오류가 발생할 수 있습니다. 본 논문에서는 짧은 행동 프로브를 통해 레이어 조합을 선택하고, 표현 정렬, 교차 스케일 증류, 스케줄링된 학생 정책 감독, LoRA 미세 조정을 통해 가지치기된 모델을 복원하는 점진적 프레임워크인 X-AuT를 소개합니다. 언어 모델 백본은 고정된 상태를 유지하며, 어텐션 LoRA 어댑터와 연결된 출력 임베딩은 증류 과정에서 적응합니다. 훈련은 전사 일관성 파이프라인에서 가장 높은 일치도를 보이는 계층을 사용하며, 이후 미세 조정 중에는 소스 재가중치를 적용합니다. 10개의 공개 중국어-영어 벤치마크에서 Qwen3-ASR-0.6B의 오디오 인코더 레이어를 18개에서 16개로 압축하면 매크로 평균 오류가 5.61%에서 5.27%로 감소합니다. 14개 레이어 모델은 20.7% 더 적은 오디오 타워 파라미터로 5.75%의 오류율을 달성합니다. 동일한 레시피에서 1.7B 교사 모델은 5.55%의 평균 오류를 보인 반면, 자가 증류는 8.45%를 기록했으며, 점진적 18→14 가지치기(5.75%)는 직접 가지치기(6.73%)보다 우수한 성능을 보였습니다. 이러한 단일 실행 결과는 두 가지 실용적인 작동 지점을 설정하고 정확도 효과가 벤치마크마다 다르게 나타남을 보여줍니다.

## 🔍 Methods & Results
- X-AuT는 전사 일치도를 기준으로 훈련 예시를 필터링하고, 짧은 행동 프로브를 통해 복구 가능한 레이어 조합을 식별하며, 3단계 훈련 스케줄을 통해 가지치기된 모델을 점진적으로 복원하는 프레임워크이다.
- 입력 파형은 N-레이어 오디오 인코더(Eθ)와 브릿지(Bφ)에 의해 오디오 임베딩으로 인코딩되며, 이 임베딩은 인과적 언어 모델 디코더(Dψ)가 전사 토큰을 예측하는 데 사용된다.
- 가지치기 작업은 인코더 블록의 순서가 지정된 부분 집합을 유지하며, 목표 깊이에서 총 텍스트 오류율(TER)을 최소화하는 복구 가능한 부분 집합과 파라미터를 찾는 것을 목표로 한다.
- 사전 훈련된 디코더(Dψ) 가중치는 고정되며, 디코더의 q/k/v/o 어텐션 프로젝션에 연결된 LoRA 파라미터와 연결된 출력 임베딩(Hω)은 증류 과정에서 훈련 가능하다.
- 복구는 중간 및 브릿지 표현을 정렬한 다음, 교사 강제 및 학생 생성 접두사 하에서 토큰 분포를 적응시키는 방식으로 이루어진다.
- 훈련 데이터는 원본 전사 및 두 ASR 가설 간의 일치도를 기반으로 9개 계층으로 필터링되며, 증류 단계에서는 가장 높은 일치도를 보이는 Class 1 데이터가 사용된다.
- 가지치기는 18→16→14 레이어의 두 단계로 진행되며, 각 단계에서 후보 레이어 조합은 고정된 5개 벤치마크 개발 스위트에서 가장 낮은 총 TER을 기준으로 선택된다.
- 학생 모델은 가지치기된 Qwen3-ASR-0.6B이고, 교사 모델은 24개 레이어 오디오 인코더를 가진 Qwen3-ASR-1.7B이며, 교사 파라미터는 추론 시 고정 및 폐기된다.
- 훈련은 3단계 레시피를 따른다: Stage 0(증류 에포크의 5%)은 중간 레이어, 브릿지, 로짓, 전사 손실을 결합하고, Stage 1은 중간 레이어 손실을 비활성화하고 브릿지 정렬, 교사 강제 로짓 KD, 골드 전사 CE를 사용하며, Stage 2는 Stage 1의 최적 체크포인트로 초기화하여 골드 전사 CE를 최적화하고 대상 도메인 소스에 대한 데이터 재가중치를 적용한다.
- Qwen3-ASR-0.6B를 18개에서 16개 오디오 인코더 레이어로 압축했을 때, 10개 중국어-영어 벤치마크에서 매크로 평균 오류가 5.61%에서 5.27%로 감소했다.
- 14개 레이어 모델은 20.7% 더 적은 오디오 타워 파라미터로 5.75%의 오류율을 달성했다.
- 1.7B 교사 모델은 5.55%의 평균 오류를 보였고, 자가 증류는 8.45%를 기록하여 X-AuT 방식이 더 우수함을 입증했다.
- 점진적 18→14 가지치기(5.75%)는 직접 가지치기(6.73%)보다 우수한 성능을 보였다.
- 정확도 효과는 벤치마크마다 다르게 나타나며, 이러한 결과는 두 가지 실용적인 작동 지점을 확립한다.

## 🖼 Figures
![Figure 1:Per-benchmark accuracy preservation of the Stage 2 X-AuT models relative to unpruned Qwen3-ASR-0.6B. The dashed contour marks the baseline (100); higher values indicate better accuracy retention.](../images/2026-09-11/2609.11412/2609.11412_fig0.png)
...
![Figure 5:Layer-screening results for single- and double-layer removals after the same 0.3-epoch warm-up. Lower TER is better; green marks the candidate selected for progressive pruning.](../images/2026-09-11/2609.11412/2609.11412_fig4.png)
...
![Figure 7:Stage 1 strategy dashboard for plain off-policy, matched off-policy, and hybrid on-policy training; lower TER is better. Exact best and final values in the accompanying text are taken from the archived checkpoint logs.](../images/2026-09-11/2609.11412/2609.11412_fig6.png)
...
---
**Usage Info**: 4289 tokens used.
**Generated at**: 2026-09-11 17:31:06
...
## 🌏 Abstract (번역)
연속 및 이산 흐름 매칭은 일반적으로 별개의 구성으로 취급됩니다. 본 논문은 이들 간의 이중성을 식별합니다: 원-핫 타겟을 가진 연속 볼록 보간 경로를 위치별 argmax를 통해 투영하면 이산 볼록 보간 경로가 생성됩니다. 이 결과는 적절한 좌표 대칭 및 경계 규칙성을 가진 소스 법칙을 필요로 하며, 연속 소스 분포를 범주형 생성을 위한 명시적인 설계 선택으로 만듭니다. 우리는 가우시안, 경계 균일, 그리고 중심 음수 지수 소스에 대해 유도된 이산 보간 동작을 도출하여, 다른 소스 기하학이 질적으로 다른 전환 타이밍과 어휘 크기 의존성을 초래함을 보여줍니다. 작은 시각적 진단과 짧은 언어 모델링 파일럿은 이러한 소스 설계 효과가 학습된 전송 및 초기 생성 품질에서도 나타날 수 있음을 시사합니다.

## 🔍 Methods & Results
- 연속 볼록 보간 경로를 원-핫 타겟과 함께 위치별 argmax를 통해 투영하여 이산 볼록 보간 경로를 생성함으로써 연속 및 이산 흐름 매칭 간의 이중성을 발견했습니다.
- 이 방법은 적절한 좌표 대칭 및 경계 규칙성을 가진 소스 법칙을 필요로 하며, 연속 소스 분포를 범주형 생성을 위한 명시적인 설계 선택으로 활용합니다.
- 가우시안, 경계 균일, 중심 음수 지수 소스에 대해 유도된 이산 보간 동작을 도출했습니다.
- 다른 소스 기하학이 질적으로 다른 전환 타이밍과 어휘 크기 의존성을 유발한다는 것을 발견했습니다.
- 작은 시각적 진단과 언어 모델링 파일럿 실험을 통해 이러한 소스 설계 효과가 학습된 전송 및 초기 생성 품질에도 영향을 미칠 수 있음을 시사했습니다.
...
=
50,257
.](../images/2026-09-11/2609.10863/2609.10863_fig0.png)
*Figure 1:Induced coefficient 
𝑘
𝑡
 for Gaussian, bounded uniform with 
...
=
𝑡
. Larger vocabularies shift the effective transition later in time.](../images/2026-09-11/2609.10863/2609.10863_fig1.png)
*Figure 2:Vocabulary-size dependence of the Gaussian induced coefficient under the linear interpolant 
𝑘
~
𝑡
...
![(a)Shifted bounded uniform source.](../images/2026-09-11/2609.10863/2609.10863_fig2.png)
*(a)Shifted bounded uniform source.*

![(a)Shifted bounded uniform source.](../images/2026-09-11/2609.10863/2609.10863_fig3.png)
*(a)Shifted bounded uniform source.*

![(b)Gaussian source.](../images/2026-09-11/2609.10863/2609.10863_fig4.png)
*(b)Gaussian source.*


---
**Usage Info**: 1159 tokens used.
**Generated at**: 2026-09-11 17:31:12
